    ...
    return (
      <div className="w-full space-y-1.5">
        ...
        <input
          ...
          aria-invalid={!!error}
          className={cn(
            'flex h-10 w-full rounded-lg border border-emerald-600 bg-white px-3 py-2 text-sm',
    ...
    ({ className, label, error, helperText, ...props }, ref) => {
        return (
            <div className="w-full space-y-1.5">
                ...
                <textarea
                    className={cn(
                        'flex min-h-[80px] w-full rounded-lg border border-emerald-600 bg-white px-3 py-2 text-sm ring-offset-emerald-600 placeholder:text-emerald-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-emerald-500 focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 transition-all shadow-sm',
                        error && 'border-red-500 focus-visible:ring-red-500',
                        className
                    )}
                    ref={ref}
                ...
                />
                {error && <p className="text-xs font-medium text-red-500">{error}</p>}
                {helperText && !error && <p className="text-xs text-emerald-300">{helperText}</p>}
            </div>
        );
    }
);
